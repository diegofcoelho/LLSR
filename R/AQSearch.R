...
#' @rdname AQSearch
#' @name AQSearch
#' @description This function allow the user to search the LLSR database to
#'  find any ATPS that matches the used criteria.
#' @export
###############################################################################
...
#' @param db.uid An Unique md5 hash Identification. User can retrieve data for
#'  a specific system if in possesion of its UID.
#' @param stacked A boolean variable used to return value as a nested list or 
#' a data.frame. Used internally to organize data output. 
...
AQSearch.default <-
  function(db = LLSR::llsr_data,
    ...
    #
    parameters <- AQSearch.Parameter(db = db, db.CompA, db.CompB, db.CompC, 
                                     db.Temp, db.ph, db.uid, stacked = TRUE)
    tielines <- AQSearch.Tieline(db = db, db.CompA, db.CompB, db.CompC,
                                 db.Temp, db.ph, db.uid, stacked = TRUE)
    binodals <- AQSearch.Binodal(db = db, db.CompA, db.CompB, db.CompC, 
                                 db.Temp, db.ph, db.uid, stacked = TRUE)
    slope <- AQSearch.Slope(db = db, db.CompA, db.CompB, db.CompC,
                            db.Temp, db.ph, db.uid, stacked = TRUE)
    #
