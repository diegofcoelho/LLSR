...
AQSearch.Parameter <-
  function(db = LLSR::llsr_data,
    ...
    # search a system that matchs the upper-phase component, if search parameter
    #  is not null.
    for (modelName in AQSysList()){
      ...
      # search a system that matchs the system's temperature, if search 
      # parameter is not null.
      if (is.valid(db.Temp)) {
        db.grep <- db.grep[(!is.na(db.grep$TEMP) & db.grep$TEMP == db.Temp), ]
      }
      # search a system that matchs the system's pH, if search parameter 
      # is not null.
      if (is.valid(db.ph)) {
        db.grep <- db.grep[(!is.na(db.grep$PH) & db.grep$PH == db.ph), ]
      }
      # search a system that matchs the system's UID, if search parameter 
      # is not null.
      if (is.valid(db.uid)) {
      ...
      if (nrow(db.grep) != 0) {
        ...
        #
        cat(paste("    [", modelName, "] ", 
                  "Your search had [", red(nrow(db.grep)), "] results.",
                  "\n", sep = ""))
      } else {
        cat(paste("    [", modelName, "] ", 
